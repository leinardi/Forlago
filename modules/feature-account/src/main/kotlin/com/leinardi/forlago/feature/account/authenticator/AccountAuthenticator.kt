...
package com.leinardi.forlago.feature.account.authenticator
...
class AccountAuthenticator @Inject constructor(
    ...
    private val isJwtExpiredInteractor: IsJwtExpiredInteractor,
    ...
    override fun addAccount(
        ...
        Timber.d("addAccount()")
        val accountsByType = accountManager.getAccountsByType(AccountAuthenticatorConfig.ACCOUNT_TYPE)
            ...
            val intent = getAuthenticatorActivityIntent(response, accountType, options, true).apply {
                putExtras(
                    bundleOf(
                        ...
                        AccountAuthenticatorConfig.KEY_IS_NEW_ACCOUNT to true,
                    ),
                )
            }
    ...
    private fun getAuthToken(
        ...
        if (accessToken.isNullOrEmpty() || isJwtExpiredInteractor(accessToken)) {
            ...
            if (!refreshToken.isNullOrEmpty() && !isJwtExpiredInteractor(refreshToken)) {
                ...
                when (val result = runBlocking { refreshAccessTokenInteractor(refreshToken) }) {
                    ...
                    is RefreshAccessTokenInteractor.Result.Failure.BadAuthentication -> Timber.w("The refresh token is not valid")
    ...
    private fun getAuthenticatorActivityIntent(
        ...
        putExtras(
            bundleOf(
                ...
                AccountManager.KEY_ACCOUNT_TYPE to accountType,
                AccountManager.KEY_USERDATA to options,
                AccountAuthenticatorConfig.KEY_IS_NEW_ACCOUNT to isNewAccount,
            ),
        )
    }
